...
namespace JetBrains.ReSharper.Psi.Secret.Parsing
{
    internal sealed class SecretMissingTokensInserter : MissingTokenInserterBase
    {
        ...
        private LeafElementBase CreateMissingToken()
        {
            ...
            if (tokenType == SecretTokenType.WHITE_SPACE || tokenType == SecretTokenType.NEW_LINE)
            {
                string text = this.myLexer.GetCurrTokenText();
                if (tokenType == SecretTokenType.WHITE_SPACE)
                {
        ...
        private sealed class EofToken : BindedToBufferLeafElement
        {
            public EofToken(IBuffer buffer)
                : base(SecretTokenType.EOF, buffer, new TreeOffset(buffer.Length), new TreeOffset(buffer.Length))
            {
            }
