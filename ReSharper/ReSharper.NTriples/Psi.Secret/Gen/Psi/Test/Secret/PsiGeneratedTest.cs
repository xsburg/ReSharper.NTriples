...
namespace ReSharper.NTriples.Impl.Tree
{
  ...
  {
      internal static void TestSecretFile (int level, ReSharper.NTriples.Tree.ISecretFile param, String caller)
    {
      if (!CanVisitFurther(param)) return;
      VisitElement (level, param, caller);
      {
        JetBrains.ReSharper.Psi.Tree.TreeNodeCollection<ReSharper.NTriples.Tree.ISentence> children = ((ReSharper.NTriples.Tree.ISecretFile)param).Sentences;
